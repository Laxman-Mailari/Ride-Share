...
def run_mongodb(i):
    try:
       client.containers.run("mongo:4.0",detach=True, name = "mongodb"+i, network="zubairapp_default",\
...
def run_slave(i):   
    try:
       ...
       print(img[0].id)
       container = client.containers.run(img[0].id,command=["sh","-c","sleep 5 && python slv.py"],detach=True,name="slv"+i,restart_policy={"Name": "on-failure"},network="zubairapp_default",environment={"type":"slave",\
       "MONGODB_DATABASE":"mymongodb","MONGODB_USERNAME":"admin","MONGODB_PASSWORD": "password","MONGODB_HOSTNAME":"mongodb"+i,"NODE_NAME":"slv_node_"+i,"type":"slave"},links={"rmq":"slv"+i,"mongodb"+i:"slv"+i})
...
def bring_up_new_worker_container(slave_name, db_name):
    ...
    client.containers.run(
        ...
        name=slave_name,
        network="zubairapp_default",
        environment={"type":"slave","NODE_NAME":"node_"+slave_name,"MONGODB_DATABASE":"mymongodb","MONGODB_USERNAME":"admin","MONGODB_PASSWORD": "password","MONGODB_HOSTNAME":db_name},
